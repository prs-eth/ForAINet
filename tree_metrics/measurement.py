...
for f_idx, filename in enumerate(matching_files):
    ...
    #veg_xloc_pre_c = np.zeros_like(veg_ins_pre, dtype=float)  #tree x location center
    #veg_yloc_pre_c = np.zeros_like(veg_ins_pre, dtype=float)  #tree y location center
    ...
    for ig, g in enumerate(un):
        ...
        # Keep increasing the height range until we have at least 10 points that satisfy the conditions
        ...
        dbh, center_x, center_y = cal_DBH_and_centerP(points_for_fitting, fig, im_path, g, min_points_threshold)
        
        #if (center_x < np.min(filtered_points[:,0]) or center_x > np.max(filtered_points[:,0])) or (center_y < np.min(filtered_points[:,1]) or center_y > np.max(filtered_points[:,1])):
        #    center_x = np.mean(filtered_points[:,0])
        #    center_y = np.mean(filtered_points[:,1])
        ...
        if np.size(idx_for_fitting1) < 10 or np.size(idx_for_fitting2) < 10:
            fig1_path = im_path.replace('fittingPointsProj', '')
            plt.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=0.5, hspace=0.5)
            fig.savefig(fig1_path, bbox_inches='tight', pad_inches=0)
            veg_Tdiamater_pre[tmp] = dbh #nsphere_dbh.sqradius #Trunk diameter
            veg_xloc_pre[tmp] = center_x
            veg_yloc_pre[tmp] = center_y
            continue
    ...
    for i in cor_veg_ins_pre[indices]:
        if i==-1:
            ...
            cor_veg_xloc_pre.append(-1)
            cor_veg_yloc_pre.append(-1)
        else:
            ...
            cor_veg_xloc_pre.append(veg_xloc_pre[inn])
            cor_veg_yloc_pre.append(veg_yloc_pre[inn])
    ...
    cor_veg_xloc_pre = np.array(cor_veg_xloc_pre)
    cor_veg_yloc_pre = np.array(cor_veg_yloc_pre)
    ...
    output = np.hstack((veg_ins_gt[indices[c_ind]].reshape(-1,1),cor_pre_ins[c_ind].reshape(-1,1),
            ...
            veg_xloc_gt[indices[c_ind]].reshape(-1,1), cor_veg_xloc_pre[c_ind].reshape(-1,1),
            veg_yloc_gt[indices[c_ind]].reshape(-1,1), cor_veg_yloc_pre[c_ind].reshape(-1,1),
            ))
    df1 = pd.DataFrame(output,
                    ...
                    columns=['gt label', 'pre label',
                                ...
                                'gt x location', 'pre x location',
                                'gt y location', 'pre y location'])

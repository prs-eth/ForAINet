...
parser.add_argument('--model_name', type=str, default='MinkUNet101')
...
class SetResNet(torch.nn.Module):
    def __init__(self): #,independent=False):
        ...
        #new scorenet
        self.backbone = MinkUNet101(in_channels=16, out_channels=1, D=3)#ResNet18(in_channels=16, out_channels=1, D=3)
        self.backbone.final = nn.Identity()
        self.backbone.relu = MinkowskiEngine.MinkowskiPooling.MinkowskiGlobalMaxPooling()
        self.classifier = Seq().append(torch.nn.Linear(384, 1)).append(torch.nn.Sigmoid())
        
        self.independent = args.independent
        if not self.independent:
            #self.att = nn.MultiheadAttention(16, 4,batch_first=False,add_bias_kv=False)
            self.att = nn.MultiheadAttention(384, 24,batch_first=False,add_bias_kv=False)

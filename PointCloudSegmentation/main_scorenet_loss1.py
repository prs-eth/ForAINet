...
parser.add_argument('--model_name', type=str, default='v4')
...
class SetResNet(torch.nn.Module):
    ...
    def forward(self,pos3d,coords_input,back_feat_input,sem_feat_input,score_gt_input,score_pre,group,all_clusters,inference=False,shuffle_groups=False):
        ...
        if self.independent:
            ...
            #cluster_scores = self.classifier(scatter_features.to(device)).squeeze(-1) # [num_cluster, 1]
        else:
            ...
            for set in uniq:
                ...
                unrelated_features = cluster_feats[idx]
            ...
            #cluster_scores = self.classifier(all_related_features.to(device)).squeeze(-1) # [num_cluster, 1]
...
def cal_score_focal_loss(
    ...
    ious = gt_scores
    with torch.no_grad():
        weight = torch.pow(torch.abs(cluster_scores-ious),2)
    #weight.requires_grad=False 
    return torch.nn.functional.binary_cross_entropy(cluster_scores, ious, weight=weight)

def cal_score_weightbyGt_loss(
    gt_scores: torch.Tensor,
    predicted_clusters: List[torch.Tensor],
    cluster_scores: torch.Tensor,
    batch: torch.Tensor,
    ...
    return torch.nn.functional.binary_cross_entropy(cluster_scores, ious, weight=ious)
...
for i in range(100):
    ...
    for step,(pos3d,coords,back_feat,sem_feat,score_gt,score_pre,group,all_clusters, gt_ins, gt_sem, batch_idx, num_ins) in enumerate(tqdm(data_loader)):
        ...
        loss = cal_score_weightbyGt_loss( #cal_score_focal_loss( #cal_score_loss(
                score_gt,
                all_clusters,

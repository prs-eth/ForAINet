...
class Trainer:
    ...
    def _initialize_trainer(self):
        ...
        if self._dataset.train_dataset:
            self._model.verify_data(self._dataset.train_dataset[0])
            ###xbb
            attr = getattr(self._dataset.train_dataset, 'weight_classes', None)
            if attr is not None:
                setattr(self._model, "weight_classes", self._dataset.train_dataset.weight_classes.to(self._device))
        else:

...
class NPM3DOriginal_4class(InMemoryDataset):
    ...
    def __init__(
        ...
        super(NPM3DOriginal_4class, self).__init__(root, transform, pre_transform, pre_filter)
        if split == "train":
...
class NPM3D_4classSphere(NPM3DOriginal_4class):
...
class NPM3D_4classCylinder(NPM3D_4classSphere):
    def _get_random(self):
        # Random spheres biased towards getting more low frequency classes
        chosen_label = np.random.choice(self._labels, p=self._label_counts)
        valid_centres = self._centres_for_sampling[self._centres_for_sampling[:, 4] == chosen_label]
...
class NPM3D_4classDataset(BaseDataset):
    ...
    def __init__(self, dataset_opt):
        ...
        dataset_cls = NPM3D_4classCylinder if sampling_format == "cylinder" else NPM3D_4classSphere

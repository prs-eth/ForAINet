...
class STPLS3DOriginalFused(InMemoryDataset):
    ...
    def processed_dir(self):
        ...
        if isinstance(self.test_area[0], int):
            ...
            return osp.join(self.root,'processed_'+str(self.grid_size)+'_'+''.join( os.path.split(f)[-1].split('.')[0] for f in self.test_area)+'_val')
    ...
    def process_test(self, test_area):
        ...
        for i, file_path in enumerate(test_area):
            ...
            if not os.path.exists(processed_area_path): 
                ...
                #)
                rgb_norm = rgb.float() / 255.0
                data = Data(pos=xyz, rgb=rgb_norm, y=semantic_labels)
    ...
    def _load_data(self, path):
        ...
        if self._sample_per_epoch > 0:
            ...
            for i, data in enumerate(self._datas): #for each test data file's data
                ...
                # self._test_spheres is list of grid-cylinder-sampled data from all test data
                ...
                # saves how many cylinders were sampled from each test data file respectively

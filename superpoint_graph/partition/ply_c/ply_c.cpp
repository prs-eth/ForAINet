...
{//compute the following geometric features (geof) features of a point cloud:
 //linearity planarity scattering verticality
    ...
    std::vector< std::vector< float > > geof(n_ver, std::vector< float >(14,0));
    ...
    {//each point can be treated in parallell independently
        ...
        // 
        ei::MatrixXf pi_minus_p = position.rowwise() - position.row(0);
        ...
        //new features from timo paper (https://ethz.ch/content/dam/ethz/special-interest/baug/igp/photogrammetry-remote-sensing-dam/documents/pdf/timo-jan-isprs2016.pdf)
        float Sum  = sqrtf(lambda[0]) + sqrtf(lambda[1]) + sqrtf(lambda[2]);
        float Omnivariance = cbrtf(sqrtf(lambda[0]) * sqrtf(lambda[1]) * sqrtf(lambda[2]));
        float Eigenentropy = - (sqrtf(lambda[0]) * log(sqrtf(lambda[0])) + sqrtf(lambda[1]) * log(sqrtf(lambda[1])) + sqrtf(lambda[2]) * log(sqrtf(lambda[2])));
        ...
        float Surface_var = sqrtf(lambda[2]) / Sum; //Change of curvature
        ...
        float Verticality2 = 1 - fabsf(v3[2]);
        ei::Vector3f eigen_v1(v1[0], v1[1], v1[2]);
        ei::Vector3f eigen_v2(v2[0], v2[1], v2[2]);
        ei::Vector3f eigen_v3(v3[0], v3[1], v3[2]);
        ei::MatrixXf v_moment1_1 = pi_minus_p * eigen_v1;
        float moment1_1 = v_moment1_1.sum();  //1st order, 1st axis
        ei::MatrixXf v_moment1_2 = pi_minus_p * eigen_v2;
        float moment1_2 = v_moment1_2.sum();  //1st order, 2st axis
        v_moment1_1 = v_moment1_1.array().square();
        float moment2_1 = v_moment1_1.sum();  //2st order, 1st axis
        v_moment1_2 = v_moment1_2.array().square();
        float moment2_2 = v_moment1_2.sum();  //2st order, 2st axis
        ...
        geof[i_ver][0] = Sum;
        geof[i_ver][1] = Omnivariance;
        geof[i_ver][2] = Eigenentropy;
        geof[i_ver][3] = Anisotropy;
        geof[i_ver][4] = planarity;
        geof[i_ver][5] = linearity;
        geof[i_ver][6] = Surface_var;
        geof[i_ver][7] = scattering;
        geof[i_ver][8] = verticality;
        geof[i_ver][9] = Verticality2;
        geof[i_ver][10] = moment1_1;
        geof[i_ver][11] = moment1_2;
        geof[i_ver][12] = moment2_1;
        geof[i_ver][13] = moment2_2;
        //---progression---
